...
class RichRenderer:
    ...
    def banner(
        ...
        *,
        ...
        self.console.rule(style="#414868")
        self.console.print()
    ...
    def _flush_markdown(self, *, title: str) -> None:
        ...
        if text.strip():
            self.console.print(
                _output_panel(
                    Markdown(text),
                    title=Text(title, style="bold #7dd3fc"),
                    border_style="#3f3f46",
                )
            )
    ...
    def _update_live_markdown(self) -> None:
        ...
        renderable = _output_panel(
            Markdown(text),
            title=Text("Assistant Output", style="bold #7dd3fc"),
            border_style="#3f3f46",
        )
    ...
    def _identity_panel(self, *, version: str, api_key_configured: bool) -> Table:
        logo = Text()
        for i, row in enumerate(_NEXUS_LOGO):
            if i:
                logo.append("\n")
            logo.append(row, style=f"bold {_LOGO_GRADIENT[i]}")
        identity = Text()
        identity.append("NexusCLI ", style="bold white")
    ...
    def _release_panel(self, *, version: str) -> Panel:
        notes = Text()
        for line in [
            "ReAct, Plan, and Multi-Agent modes in one CLI",
            ...
            notes.append("\n")
        notes.append("/help", style="#c084fc")
        notes.append(" for more", style="dim")
        return Panel(
            notes,
            title=Text(f"What's new (v{version})", style="bold #c084fc"),
            border_style="#414868",
            box=box.ROUNDED,
            padding=(0, 2),
        )
